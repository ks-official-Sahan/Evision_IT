  ...
  useEffect(() => {
    ...
    const onMouseEnterMagnetic = (e: Event) => {
      ...
      const offsetX = Math.min(
        Math.max(centerX - mouseEvent.clientX, -maxOffset),
        maxOffset,
      );
      const offsetY = Math.min(
        Math.max(centerY - mouseEvent.clientY, -maxOffset),
        maxOffset,
      );

      gsap.to(cursor, {
        x: mouseEvent.clientX + offsetX * 0.3,
        y: mouseEvent.clientY + offsetY * 0.3,
        duration: 0.5,
        ease: "power2.out",
      });
    };
      ...
      {Array.from({ length: 5 }).map((_, index) => (
        <div
          ...
          style={{
            ...
            boxShadow: `0 0 ${15 - index * 2}px rgba(255,255,255,${
              0.5 - index * 0.08
            })`,
          }}
        />
      ...
      {Array.from({ length: 8 }).map((_, index) => (
        <div
          ...
          style={{
            ...
            boxShadow: `0 0 ${8 + index * 2}px rgba(6,182,212,${
              0.3 - index * 0.03
            })`,
          }}
        />
